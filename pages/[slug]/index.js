...
function Portfolio({ page, totalPages, error }) {
    ...
    useEffect(() => {
        /* Преобразование урла роутера нужно на стадии монтирования компонента,
         чтобы можно было получить данные из урла и синхронизировать с UI. Т.е. пользователь переходит по ссылке, которая уже имеет значения фильтра,
         и в UI нужные checkbox будут иметь значение true */
        preventSearchUrl();
    }, [dispatch]);
